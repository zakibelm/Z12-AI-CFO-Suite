...
# ─────────────────────────────────────────────────────────
# Item 5 — Optimisation tokens : routing par complexité
# ─────────────────────────────────────────────────────────

# Modèle ultra-rapide pour questions simples (FREE)
FAST_MODEL: str = "google/gemini-2.0-flash-exp:free"
# Modèle intermédiaire pour questions moyennes (FREE)
MEDIUM_MODEL: str = "deepseek/deepseek-r1:free"

# Mots-clés signalant une question COMPLEXE → modèle premium
_COMPLEX_KEYWORDS = {
    "dcf", "valorisation", "acquisition", "due diligence", "fusion",
    "restructuration", "dérivé", "options", "contrat", "litige",
    "rs&de", "rsd", "recherche développement", "audit complet",
    "analyse complète", "rapport détaillé", "étude approfondie",
    "modèle financier", "projection 5 ans", "business plan",
}
# Mots-clés signalant une question SIMPLE → modèle rapide
_SIMPLE_KEYWORDS = {
    "c'est quoi", "qu'est-ce que", "définition", "expliquer",
    "kesako", "comment fonctionne", "quel est le taux",
    "c'est combien", "bonjour", "merci", "salut", "aide",
    "résume", "liste", "quels sont", "quelles sont",
}

def assess_complexity(query: str) -> str:
    """Classe la question en simple/medium/complex selon les mots-clés.
    
    Returns: 'simple' | 'medium' | 'complex'
    """
    q = query.lower()
    # Vérifie complexe d'abord (priorité haute)
    if any(kw in q for kw in _COMPLEX_KEYWORDS):
        return "complex"
    # Vérifie simple
    if any(kw in q for kw in _SIMPLE_KEYWORDS) or len(query.split()) < 8:
        return "simple"
    return "medium"


def select_model(agent: str, budget_remaining_pct: float, query: str = "") -> str:
    """Sélectionne le modèle selon complexité + agent + budget (FREE FIRST).
    
    Règles (par ordre de priorité) :
    1. Budget < 5%  → FAIL (géré par l'appelant)
    2. Budget < 20% → BUDGET_SAVE_MODEL (sauf agents financiers critiques)
    3. Agents financiers critiques → toujours Claude (Loi 25)
    4. Complexité 'simple' → FAST_MODEL (Gemini Flash Free)
    5. Complexité 'medium'  → MEDIUM_MODEL (DeepSeek R1 Free)
    6. Complexité 'complex' → modèle par défaut de l'agent
    7. Sinon → AGENT_DEFAULT_MODEL ou BUDGET_SAVE_MODEL
    """
    # Agents financiers critiques : toujours le meilleur modèle (compliance Loi 25)
    z12_financial_agents = {"CFO", "TaxAgent", "AccountingAgent", "Sophie", "Isabelle", "Alexandre"}
    if agent in z12_financial_agents:
        return AGENT_DEFAULT_MODEL.get(agent, ESCALATION_MODEL)

    # Budget serré → économique
    if budget_remaining_pct < 20:
        return BUDGET_SAVE_MODEL

    # Routing par complexité de la question
    if query:
        ...
        if complexity == "simple":
            ...
            return MEDIUM_MODEL
        # complex → fall through to agent default
