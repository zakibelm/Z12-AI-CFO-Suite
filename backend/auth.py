...
def _is_valid_uuid(s: str) -> bool:
    # Accept integer strings (local auth user IDs) or UUID format
    if s and s.isdigit():
        return True
    try:
        uuid.UUID(s)
...
async def get_current_user_id(
    ...
    # Bypass pour appels internes WhatsApp
    if x_internal_service == "whatsapp":
        return "whatsapp-service-user"
    """
    ...
    # 2.5 Cookie-based auth from local auth (phoenix_session cookie)
    try:
        import jwt as _jwt
        import os as _os
        ...
        if _cookie_token and _session_secret:
            try:
                _p = _jwt.decode(_cookie_token, _session_secret, algorithms=["HS256"], audience="phoenix")
                _uid = str(_p.get("sub") or _p.get("user_id") or "")
                if _uid:
                    log.debug("auth.cookie.ok", user_id=_uid)
                    return _uid
            except Exception as _e:
                log.debug("auth.cookie.invalid", error=str(_e))
    except Exception:
        pass
